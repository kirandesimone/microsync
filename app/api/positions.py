...
def parse_iso_timestamp(timestamp: str) -> datetime:
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid timestamp format: {timestamp}"
        )
...
def parse_user_posts(
    snapshot: dict[str, PositionRecord],
    area_id: str,
...
async def get_user_posts(
    area_id: str, since: Optional[str] = Query(default=None)
):
    try:
        snapshot = position_read_cache.get_many()
        since_dt = None
        ...
        entries = parse_user_posts(snapshot, area_id, since_dt)
